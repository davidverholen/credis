...
class Client
{
     ...
     * @var array
     */
    protected $wrapperMethods
        = array('delete' => 'del', 'getkeys' => 'keys', 'sremove' => 'srem');
    ...
    public function __construct(
        $host = '127.0.0.1', $port = 6379, $timeout = null, $persistent = '',
        $db = 0, $password = null
    ) {
    ...
    public function forceStandalone()
    {
        if ($this->connected) {
            throw new CredisException(
                'Cannot force Credis_Client to use standalone PHP driver after a connection has already been established.'
            );
        }
    ...
    protected function convertHost()
    {
        if (preg_match('#^(tcp|unix)://(.*)$#', $this->host, $matches)) {
            if ($matches[1] == 'tcp') {
                if (!preg_match(
                    '#^([^:]+)(:([0-9]+))?(/(.+))?$#', $matches[2], $matches
                )
                ) {
                    throw new CredisException(
                        'Invalid host format; expected tcp://host[:port][/persistence_identifier]'
                    );
                }
                $this->host = $matches[1];
    ...
    public function connect()
    {
        ...
        if ($this->standalone) {
            ...
            $result = $this->persistent
                ? $this->redis->pconnect(
                    $this->host, $this->port, $this->timeout, $this->persistent
                )
                : $this->redis->connect(
        ...
        if (!$result) {
            ...
            throw new CredisException(
                "Connection to Redis failed after $failures failures."
                . (isset($errno) && isset($errstr)
                    ? "Last Error : ({$errno}) {$errstr}" : "")
            );
        }
    ...
    public function setReadTimeout($timeout)
    {
        if ($timeout < -1) {
            throw new CredisException(
                'Timeout values less than -1 are not accepted.'
            );
        }
        ...
        if ($this->connected) {
            if ($this->standalone) {
                $timeout
                    = $timeout <= 0 ? 315360000 : $timeout; // Ten-year timeout
                stream_set_blocking($this->redis, true);
                stream_set_timeout(
                    $this->redis, (int)floor($timeout),
    ...
    public function close()
    {
        ...
        if ($this->connected && !$this->persistent) {
            try {
                $result = $this->standalone ? fclose($this->redis)
                    : $this->redis->close();
    ...
    public function pSubscribe($patterns, $callback)
    {
        if (!$this->standalone) {
            return $this->__call(
                'pSubscribe', array((array)$patterns, $callback)
            );
        }
        ...
        while ($patternCount--) {
            if (isset($status)) {
                ...
                list($command, $pattern, $status) = $this->__call(
                    'psubscribe', array($patterns)
                );
            }
            if (!$status) {
    ...
    public function subscribe($channels, $callback)
    {
        if (!$this->standalone) {
            return $this->__call(
                'subscribe', array((array)$channels, $callback)
            );
        }
        ...
        while ($channelCount--) {
            if (isset($status)) {
                ...
                list($command, $channel, $status) = $this->__call(
                    'subscribe', array($channels)
                );
            }
            if (!$status) {
    ...
    public function __call($name, $args)
    {
        ...
        if ($this->standalone) {
            ...
            if ($this->usePipeline) {
                if ($name == 'pipeline') {
                    throw new CredisException(
                        'A pipeline is already in use and only one pipeline is supported.'
                    );
                } else {
                    if ($name == 'exec') {
                        if ($this->isMulti) {
                            $this->commandNames[] = $name;
                            $this->commands .= self::_prepare_command(
                                array($this->getRenamedCommand($name))
                            );
                        }
            ...
            switch ($name) {
                ...
                case 'evalsha':
                    ...
                    $args = array($args[0], array_merge($cKeys, $cArgs),
                                  count($cKeys));
                    break;
                case 'subscribe':
                case 'psubscribe':
            ...
            try {
                ...
                if ($name == 'pipeline' || $name == 'multi') {
                    if ($this->isMulti) {
                        ...
                        $this->isMulti = true;
                        $this->redisMulti = call_user_func_array(
                            array($this->redis, $name), $args
                        );
                    }
                } else {
                ...
                if ($this->isMulti) {
                    call_user_func_array(
                        array($this->redisMulti, $name), $args
                    );
                    return $this;
                }
    ...
    protected function write_command($command)
    {
        ...
        if (feof($this->redis)) {
            ...
            if (($this->isMulti && !$this->usePipeline) || $this->isWatching) {
                ...
                throw new CredisException(
                    'Lost connection to Redis server during watch or transaction.'
                );
            }
            $this->connected = false;
            $this->connect();
    ...
    protected function read_reply($name = '')
    {
        ...
        if ($reply === false) {
            ...
            if ($info['timed_out']) {
                throw new CredisException(
                    'Read operation timed out.', CredisException::CODE_TIMED_OUT
                );
            } else {
                $this->connected = false;
                throw new CredisException(
        ...
        switch ($replyType) {
            ...
            case '-':
                if ($this->isMulti || $this->usePipeline) {
                    ...
                    if ($name == 'evalsha'
                        && substr($reply, 0, 9) == '-NOSCRIPT'
                    ) {
                        $response = null;
                    } else {
                        throw new CredisException(
                            substr($reply, 0, 4) == '-ERR' ? substr($reply, 5)
                                : substr($reply, 1)
                        );
                    }
                }
            ...
            default:
                throw new CredisException(
                    'Invalid response: ' . print_r($reply, true)
                );
                break;
        }
    ...
    {
        return sprintf(
            '*%d%s%s%s', count($args), CRLF,
            implode(array_map(array('self', '_map'), $args), CRLF), CRLF
        );
    }
